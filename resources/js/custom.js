...
(function($) {
    ...
    var pages = {
        ...
        ----------------------------------------------*/
            ...
            pages.selectnav.children().removeAttr('selected');
            var selector=pages.currentPage.attr('id');
            selector='#'+selector;
            pages.selectnav.val(selector);
        },
        prepare:function(){
            var self=this;
            self.config.height= $(window).height();
            self.config.windowWidth=$(window).width();
            self.config.desktopFlag = (self.config.windowWidth < self.config.responsiveWidth) ? false : true;
            self.sections.css('height', self.config.height);
            self.wrapper.css('height', self.config.height);
            self.sections.children('.inner-wrapper').css('height', self.config.height);

            var buttonHeight = self.config.height - self.config.navHeight,
                navObj={};


            self.dropdown();
            if (self.toTop) {
                navObj = {top:buttonHeight};
            } else {
                navObj = {top: 0};
            }
            ...
            innerWrapper.css('height',self.config.height-self.config.navHeight);
    ...
    function showContact() {
        ...
        if (!contactSection.hasClass('activated')){
          ...
          overlay:{
            ...
            address: "Danyla Halytskoho boulevard, Ternopil",
            options:{
